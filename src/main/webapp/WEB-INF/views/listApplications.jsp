    ...
    <script>
     ...
     // ========== REPORT FUNCTIONS FOR APPLICATIONS ==========
        ...
        function getApplicationData() {
            var data = [['SR. NO.', 'INTERNSHIP', 'STUDENT NAME', 'STATUS']];
            var rows = document.querySelectorAll('#applicationsTable tbody tr');
            
            for (var i = 0; i < rows.length; i++) {
                var cells = rows[i].querySelectorAll('td');
                if (cells.length >= 4) {
                    // Get status and clean it (remove HTML tags and extra spaces)
                    var statusCell = cells[3];
                    var statusText = '';
                    if (statusCell) {
                        // Get the text from the badge span
                        var badgeSpan = statusCell.querySelector('.badge-custom');
                        if (badgeSpan) {
                            statusText = badgeSpan.innerText.trim();
                        } else {
                            statusText = statusCell.innerText.trim();
                        }
                        // Clean up status text
                        statusText = statusText.replace(/Applied/i, 'Applied');
                        statusText = statusText.replace(/Shortlisted/i, 'Shortlisted');
                        statusText = statusText.replace(/Selected/i, 'Selected');
                        statusText = statusText.replace(/Rejected/i, 'Rejected');
                    }
        ...
        function exportToPDF() {
            ...
            var html = '<!DOCTYPE html><html><head><title>Applications Report</title><style>';
            ...
            html += '.status-applied{color:#4B8BBE;font-weight:bold}';
            html += '.status-shortlisted{color:#06b6d4;font-weight:bold}';
            html += '.status-selected{color:#10b981;font-weight:bold}';
            html += '.status-rejected{color:#ef4444;font-weight:bold}';
            html += '</style></head><body>';
            html += '<div class="header">';
            html += '<h1>SmartIntern - Internship Applications Report</h1>';
            html += '<p>Generated on: ' + new Date().toLocaleString() + '</p>';
            html += '<p>Total Applications: ' + (data.length - 1) + '</p>';
            html += '</div>';
            html += '<table><thead><tr>';
            html += '<th>SR. NO.</th><th>INTERNSHIP</th><th>STUDENT NAME</th><th>STATUS</th>';
            html += '</thead><tbody>';
            ...
            for (var i = 1; i < data.length; i++) {
                ...
                if (status === 'Applied' || status.includes('APPLIED')) statusClass = 'status-applied';
                else if (status === 'Shortlisted' || status.includes('SHORTLISTED')) statusClass = 'status-shortlisted';
                else if (status === 'Selected' || status.includes('SELECTED')) statusClass = 'status-selected';
                else if (status === 'Rejected' || status.includes('REJECTED')) statusClass = 'status-rejected';
                
                html += '<tr>';
                html += '<td>' + (row[0] ? row[0] : '') + '</td>';
                html += '<td>' + (row[1] ? row[1] : '') + '</td>';
                html += '<td>' + (row[2] ? row[2] : '') + '</td>';
        ...
        function exportToExcel() {
            ...
            link.download = 'Applications_Report_' + new Date().toISOString().slice(0,10) + '.xls';
            link.click();
        ...
        function exportToWord() {
            ...
            var html = '<html><body>';
            html += '<h1>Internship Applications Report</h1>';
            ...
            var link = document.createElement('a');
            link.href = URL.createObjectURL(blob);
            link.download = 'Applications_Report_' + new Date().toISOString().slice(0,10) + '.doc';
            link.click();
            URL.revokeObjectURL(link.href);
        }
        ...
        function printReport() {
            ...
            var win = window.open('', '_blank');
            var html = '<html><head><title>Print Applications Report</title>';
