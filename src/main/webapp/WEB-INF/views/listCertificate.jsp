...
<head>
  ...
  <title>SmartIntern · Certificate List</title>
  ...
  <style>
    ...
    * {
    ...
    .badge-verified[style*="background:#fff0db"] {
    ...
    .certificate-card div[style*="font-size: 2rem"] {
      color: var(--pure-white) !important;
      font-size: 2rem !important;
      font-weight: 700 !important;
    }

    .certificate-card div[style*="color: #0f9d8e"] {
      color: #34d399 !important;
    }

    .certificate-card div[style*="color: #f6b83e"] {
      color: #fbbf24 !important;
    }

    .certificate-card div[style*="color: #0a7e3a"] {
      color: #34d399 !important;
    }
    ...
    /* ========================================================
       DATATABLES CUSTOMIZATION - FIXED PAGINATION STYLING
       ======================================================== */
...
<body>
  ...
  <div class="dashboard-layout" id="dashboardLayout">
    ...
    <!-- MAIN PANEL - CERTIFICATE LIST CONTENT (UNTOUCHED LOGIC) -->
    <main class="main-panel" id="mainPanel">
      ...
      <div class="table-container">
        <table id="certificateTable" class="table table-hover">
          ...
          <tbody>
            <c:forEach items="${certificateList}" var="cert" varStatus="status">
              <tr>
                ...
                <td>
                  <div class="d-flex align-items-center gap-2">
                    ...
                    <div>
                      <strong style="color: var(--pure-white);">${cert.enrollment.student.firstName} ${cert.enrollment.student.lastName}</strong><br>
                      <small class="text-muted">${cert.enrollment.student.email}</small>
                    </div>
                  </div>
                </td>
                <td>${cert.enrollment.internship.title}</td>
                <td>${cert.issuedDate}</td>
                <td>
                  <span class="certificate-code">${cert.verificationCode}</span>
                </td>
                <td>
                  <c:choose>
                    ...
                    <c:otherwise>
                      <span class="badge-verified" style="background: rgba(245, 158, 11, 0.15); color: #fbbf24; border: 1px solid rgba(245, 158, 11, 0.3);">
                ...
                <td>
                  <div class="action-buttons">
                    <a href="viewCertificate?certificateId=${cert.certificateId}" class="action-btn" title="View">
                      <i class="bi bi-eye-fill"></i>
                    </a>
                    <a href="downloadCertificate?certificateId=${cert.certificateId}" class="action-btn" title="Download">
                      <i class="bi bi-download"></i>
                    </a>
                    <c:if test="${empty cert.certificateURL}">
                      <a href="uploadCertificate?certificateId=${cert.certificateId}" class="action-btn" title="Upload">
                        <i class="bi bi-cloud-upload"></i>
                      </a>
                    </c:if>
                    <a href="deleteCertificate?certificateId=${cert.certificateId}" class="action-btn delete" title="Delete" 
            ...
            <c:if test="${empty certificateList}">
              <tr>
                <td colspan="7" class="text-center py-5">
                  ...
                  <p class="text-muted">Click "Issue New Certificate" to create your first certificate</p>
                </td>
              </tr>
            </c:if>
          </tbody>
      ...
      <div class="row g-4 mt-2">
        <div class="col-md-3">
          <div class="certificate-card">
            <h6 style="color: var(--soft-blue);">TOTAL CERTIFICATES</h6>
            <div style="font-size: 2rem; font-weight: 700; color: var(--pure-white);">${totalCertificates}</div>
          </div>
        </div>
        <div class="col-md-3">
          <div class="certificate-card">
            <h6 style="color: var(--soft-blue);">ISSUED THIS MONTH</h6>
            <div style="font-size: 2rem; font-weight: 700; color: #34d399;">${monthlyCertificates}</div>
          </div>
        </div>
        <div class="col-md-3">
          <div class="certificate-card">
            <h6 style="color: var(--soft-blue);">PENDING UPLOADS</h6>
            <div style="font-size: 2rem; font-weight: 700; color: #fbbf24;">${pendingUploads}</div>
          </div>
        </div>
        <div class="col-md-3">
          <div class="certificate-card">
            <h6 style="color: var(--soft-blue);">VERIFIED</h6>
            <div style="font-size: 2rem; font-weight: 700; color: #34d399;">${verifiedCertificates}</div>
          </div>
        </div>
      </div>
      ...
      toggleBtn.addEventListener('click', function(e) {
        ...
        setTimeout(() => {
          if ($.fn.DataTable.isDataTable('#certificateTable')) {
            $('#certificateTable').DataTable().columns.adjust().draw();
          }
        }, 300);
      });
